...
from kg_search.pdf_extraction.grobid_client import post_pdf
from kg_search.pdf_extraction.models import ExtractedPdfDocument, PdfReference
from kg_search.pdf_extraction.parser import parse_tei_document
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--env", default=str(DEFAULT_ENV_PATH), help="Path to the shared .env file.")
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument("--max-workers", type=int, default=4, help="Maximum parallel paper workers.")
    parser.add_argument(
        "--grobid-max-concurrency",
        type=int,
        default=4,
        help="Maximum concurrent GROBID PDF extraction requests.",
    )
...
class KgTitleLookupClient:
    def search_work_by_title(self, title: str, expected_year: int | None) -> dict[str, Any] | None:
        ...
        from kg_search.service import search_paper_title_candidates

        title_candidates = search_paper_title_candidates(title_text, top_k=10)
        exact_rows = title_candidates["exact"]
        exact_match = choose_best_openalex_match(
            title_text,
            expected_year,
            [
                {
        ...
        if exact_match is not None:
            exact_match["match_source"] = "kg_title_exact"
            return exact_match

        ft_rows = title_candidates["fulltext"]
        ft_match = choose_best_openalex_match(
            title_text,
            expected_year,
            [
                {
        ...
        if ft_match is not None:
            ft_match["match_source"] = "kg_title_ft"
            return ft_match
        return None
...
class PdfXmlPipeline:
    def __init__(
        ...
        *,
        ...
        self._grobid_semaphore = threading.Semaphore(max(1, config.grobid_max_concurrency))

    def run(self) -> dict[str, Any]:
        papers = load_paper_records(self.config.input_path, self.config.paper_list_path)
        target_success_count = self.config.top_k
        if target_success_count is not None:
            papers = papers[: target_success_count * 2]

        self.config.output_dir.mkdir(parents=True, exist_ok=True)
        entries = self.process_records_until_target(papers, target_success_count=target_success_count)
        ...
        if target_success_count is not None:
            selected_papers = selected_papers[:target_success_count]

        target_fulfilled = target_success_count is None or len(selected_papers) >= target_success_count
        shortfall_count = (
            max(target_success_count - len(selected_papers), 0)
            if target_success_count is not None
            else 0
        )
        ...
        manifest = {
            ...
            "candidate_pool_count": len(papers),
            "attempted_count": len(entries),
            "processed_count": len(entries),
            "success_count": success_count,
            "failed_count": len(entries) - success_count,
            "candidate_failed_count": len(entries) - success_count,
    ...
    def process_records_until_target(
        self,
        papers: list[dict[str, Any]],
        *,
        target_success_count: int | None,
    ) -> list[dict[str, Any]]:
        if target_success_count is None:
            return self.process_records(papers)
        ...
        while cursor < len(papers) and success_count < target_success_count:
            batch = papers[cursor : cursor + max_workers]
            batch_entries = self.process_records(batch, start_rank=cursor + 1)
            entries.extend(batch_entries)
            success_count += sum(1 for entry in batch_entries if entry["status"] == "ok")
            cursor += len(batch)

        return entries

    def process_records(self, papers: list[dict[str, Any]], *, start_rank: int = 1) -> list[dict[str, Any]]:
        max_workers = max(1, self.config.max_workers)
        ranked_records = list(enumerate(papers, start=start_rank))
        if max_workers == 1 or len(ranked_records) <= 1:
            return [self.process_record(rank, record) for rank, record in ranked_records]

        entries_by_rank: dict[int, dict[str, Any]] = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_rank = {
                executor.submit(self.process_record, rank, record): rank
                for rank, record in ranked_records
            }
            for future in concurrent.futures.as_completed(future_to_rank):
                rank = future_to_rank[future]
                try:
                    entries_by_rank[rank] = future.result()
        ...
        return [entries_by_rank[rank] for rank, _ in ranked_records]
    ...
    def process_record(self, rank: int, record: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            print(f"paper {rank:02d} start title={title}", file=sys.stderr, flush=True)
            append_trace(
                trace,
                step="start",
                status="ok",
            ...
            print(
                (
                    f"paper {rank:02d} pdf ok method={fetch_result.get('method')} "
                    f"size_bytes={fetch_result.get('size_bytes')}"
                ),
                file=sys.stderr,
                flush=True,
            )
            ...
            print(
                (
                    f"paper {rank:02d} done status=ok elapsed_ms={elapsed_ms} "
                    f"references={tei_result.get('reference_count')} "
                    f"sections={tei_result.get('body_section_count')}"
                ),
                file=sys.stderr,
                flush=True,
            )
            ...
            entry["error"] = str(exc)
            print(f"paper {rank:02d} error type={exc.__class__.__name__} message={exc}", file=sys.stderr, flush=True)
            append_trace(
                trace,
                step="paper",
                status="error",
    ...
    def resolve_openalex(self, record: dict[str, Any], trace: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        append_trace(
            ...
            status="running",
            message="Searching local KG by title before OpenAlex API search.",
        ...
        if kg_match is not None:
            append_trace(
                ...
                step="kg_title_lookup",
                status="ok",
                message="Resolved OpenAlex ID from local KG title lookup.",
                extra=kg_match,
            )
...
def build_config(args: argparse.Namespace) -> tuple[PipelineConfig, urllib.request.OpenerDirector]:
    ...
    config = PipelineConfig(
        ...
        max_workers=max(1, getattr(args, "max_workers", 4)),
        grobid_max_concurrency=max(1, getattr(args, "grobid_max_concurrency", 4)),
