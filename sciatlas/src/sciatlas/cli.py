...
class Spinner:
    ...
    def __init__(self, message: str, *, enabled: bool = True) -> None:
        ...
        self.enabled = enabled and sys.stderr.isatty() and not (
            os.environ.get("SCIATLAS_NO_SPINNER")
        )
...
def render_user_output(payload: dict[str, Any]) -> str:
    ...
    command = payload.get("command", "sciatlas")
    ...
    display_title = channel_titles.get(command, "SciAtlas")
...
def request_json(
    ...
    *,
    ...
    if endpoint.rstrip("/") != "/healthz":
        ...
        if not api_key:
            ...
            return {
                ...
                "error": "This endpoint requires SCIATLAS_API_KEY or --api-key.",
    ...
    spinner_message = "SciAtlas retrieval is running. Please wait; complex graph retrieval may take several seconds."
...
# - 只映射到 SciAtlas API 当前公开支持的 plan 字段：
...
def _optional_llm_config() -> dict[str, Any] | None:
    provider = (_env_first("LLM_PROVIDER", "SCIATLAS_LLM_PROVIDER") or "chat_completions").lower()
    if provider in {"0", "false", "none", "off", "disabled"}:
        return None
    api_key = _env_first("LLM_API_KEY", "SCIATLAS_LLM_API_KEY", "OPENAI_API_KEY")
    base_url = _env_first("LLM_BASE_URL", "SCIATLAS_LLM_BASE_URL", "OPENAI_BASE_URL")
    full_url = _env_first("LLM_CHAT_COMPLETIONS_URL", "SCIATLAS_LLM_CHAT_COMPLETIONS_URL", "OPENAI_CHAT_COMPLETIONS_URL")
    ...
    return {
        ...
        "headers": headers,
        "model": model,
        "timeout": int(os.environ.get("SCIATLAS_LLM_TIMEOUT") or os.environ.get("LLM_TIMEOUT") or 30),
        "temperature": float(os.environ.get("SCIATLAS_LLM_TEMPERATURE") or os.environ.get("LLM_TEMPERATURE") or 0),
        "max_tokens": int(os.environ.get("SCIATLAS_LLM_MAX_TOKENS") or os.environ.get("LLM_MAX_TOKENS") or 512),
    }
...
def optional_provider_config_status() -> dict[str, Any]:
    return {
        ...
        "openalex_configured": not _missing_or_placeholder(_env_first("OA_API_KEY", "OPENALEX_API_KEY", "SCIATLAS_OPENALEX_API_KEY")),
        "openalex_mailto_configured": not _missing_or_placeholder(_env_first("OPENALEX_MAILTO", "OPENALEX_EMAIL", "SCIATLAS_OPENALEX_MAILTO")),
        "grobid_configured": not _missing_or_placeholder(_env_first("GROBID_BASE_URL", "SCIATLAS_GROBID_BASE_URL")),
    }
...
# ============================================================
# 检索偏向超参数：前端挡位 -> SciAtlas API options.retrieval_bias
# ============================================================
...
def _report_header(command: str, title: str, now: str, query_text: str, response: dict[str, Any], ctx: dict[str, Any]) -> list[str]:
    ...
    lines = [
        f"# SciAtlas Downstream Channel Report: {command}",
        "",
        f"> {title}",
        "",
        "## 1. Task Overview",
...
def render_generic_markdown_report(*, command: str, request: dict[str, Any], response: dict[str, Any], max_items: int = 10) -> str:
    ...
    lines: list[str] = []
    lines.append("# SciAtlas Retrieval Report")
    lines.append("")
...
def _apply_channel_hint_to_report(report_path: str | None, command: str) -> None:
    ...
    try:
        ...
        header = f"# SciAtlas Downstream Channel Report: {command}\n\n> {hint}\n\n"
        if not (
            original.startswith("# SciAtlas Downstream Channel Report")
            or original.startswith("# SciAtlas Downstream Channel Report")
        ):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="sciatlas",
        usage="sciatlas [global-options] <command> [command-options]",
        description="SciAtlas CLI: scientific knowledge-graph retrieval and downstream research workflows.",
        epilog="""commands:
  health                 Check SciAtlas API health
...
tips:
  sciatlas <command> -h               show command-specific help
  export SCIATLAS_API_BASE_URL="http://127.0.0.1:8000" for local server testing
  reports are saved under runs/<run_id>/report.md""",
    ...
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL, help="SciAtlas API base URL.")
    ...
    sub = parser.add_subparsers(
        ...
        metavar="<command>",
        title="commands",
        description="Use 'sciatlas <command> -h' for command-specific options.",
    )



    p = sub.add_parser("health", help="Check SciAtlas API health.")
...
tips:
  ...
  reports are saved under runs/<run_id>/report.md"""
    ...
    #   usage: sciatlas literature-review [options]
    # rather than:
    #   usage: sciatlas [global-options] <command> [command-options] literature-review ...
    for _cmd_name, _sub_parser in sub.choices.items():
        _sub_parser.prog = f"sciatlas {_cmd_name}"
        _sub_parser.usage = f"sciatlas {_cmd_name} [options]"
        _sub_parser.formatter_class = argparse.RawDescriptionHelpFormatter

    _examples = {
        "search-papers": """examples:
  sciatlas search-papers --retrieval-mode hybrid --query "open world agent" --keyword "high:open world agent" --top-k 3
  sciatlas search-papers --retrieval-mode semantic --query "idea evaluation" --top-k 3
  sciatlas search-papers --retrieval-mode title --title "high:Voyager: An Open-Ended Embodied Agent with Large Language Models" --top-k 3
...
purpose:
  ...
  sciatlas idea-grounding --idea "LLM-based multi-perspective evaluation for scientific research ideas" --keyword "high:idea evaluation" --top-k 3
...
purpose:
  ...
  sciatlas trend-report --query "retrieval augmented generation" --keyword "high:retrieval augmented generation" --top-k 5

purpose:
  Retrieve influential papers and organize evidence for research trend analysis.""",

        "researcher-review": """examples:
  sciatlas researcher-review --author "Yoshua Bengio" --limit 10 --no-abstract
  sciatlas researcher-review --query "papers by Geoffrey Hinton" --limit 10 --no-abstract
...
purpose:
  ...
  sciatlas related-authors --query "open world agent" --keyword "high:open world agent" --top-k 5
  sciatlas related-authors --query "idea evaluation" --keyword "high:idea evaluation" --top-k 5

purpose:
  Find authors related to a research direction.""",

        "author-papers": """examples:
  sciatlas author-papers --text "author: Yoshua Bengio" --limit 10 --no-abstract
  sciatlas author-papers --text "papers by Geoffrey Hinton" --limit 10 --no-abstract

purpose:
  Retrieve papers written by a specified author.""",

        "support-papers": """examples:
  sciatlas support-papers --text "Query topic: open world agents and embodied AI. Candidate authors: Yoshua Bengio, Pieter Abbeel." --top-k-per-author 1 --no-author-stats
...
purpose:
  ...
  sciatlas paper-search --text "open world agent" --mode vector --field title --top-k 3
  sciatlas paper-search --text "idea evaluation" --mode vector --field abstract --top-k 3
