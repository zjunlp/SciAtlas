...
def _builtin_skills() -> dict[str, dict[str, Any]]:
    try:
        data = resources.files("sciatlas").joinpath("builtin_skills.json").read_text(encoding="utf-8")
...
def _dirs() -> list[Path]:
    dirs = [Path.cwd() / "skills", Path.home() / ".sciatlas" / "skills"]
    extra = os.getenv("SCIATLAS_SKILLS_DIR", "")
...
def cmd_init(skills: dict[str, dict[str, Any]], name: str, source: str | None, output: str | None, force: bool) -> int:
    if source:
        ...
        obj = {
            "name": name,
            "aliases": [],
            "description": "Custom SciAtlas skill.",
            ...
            "examples": [f"sciatlas skill run {name} --query \"open world agent\" --keyword \"high:open world agent\""]
        }
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="sciatlas skill", description="Editable downstream skills for SciAtlas CLI.")
...
def dispatch_skill_cli(argv: list[str]) -> int | list[str]:
    ...
    if args.action == "run":
        if args.name not in skills:
            ...
            print("Use `sciatlas skill list`.")
            return 2
        expanded = expand_skill(skills[args.name], list(args.extra or []))
        if args.dry_run:
            print("sciatlas " + " ".join(expanded))
            return 0
        return expanded
    return 0
